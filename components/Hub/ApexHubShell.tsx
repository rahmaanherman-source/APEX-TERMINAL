...
import {
  ...
  LockKeyhole, MonitorCog, Plus, Search, ShieldCheck, Sparkles, TerminalSquare,
  Users, Wrench, X, Zap, Palette, BarChart3, ExternalLink
} from 'lucide-react';
...
function statusLabel(status: TruthState) { return status.replaceAll('_', ' '); }
...
function ConnectedAppsPanel({ onOpen }: { onOpen: (app: ConnectedApp) => void }) {
  return <section className="hub-panel connected-panel">
    ...
    <div className="connected-list">{connectedApps.map(app => { const Icon = iconMap[app.iconKey] ?? Box; return <button className="connected-row" key={app.id} onClick={() => onOpen(app)}>
      <span className="provider-icon"><Icon size={15}/></span><span className="provider-name">{app.name}</span><span className="provider-cap">{app.capabilitySummary}</span><StatusDot status={app.status}/><span className="provider-status">{statusLabel(app.status)}</span>
    </button>; })}</div>
  </section>;
}
...
export default function ApexHubShell() {
  ...
  const [vaultState] = useState<TruthState>('UNKNOWN');
  const systemMetrics = useMemo(() => ({ cpu: 'OBSERVED', gpu: 'OBSERVED', memory: 'LOCAL', net: 'LOCAL' }), []);

  const navigate = (next: HubDestination) => { if (next === 'CHARACTER_STUDIO') { window.location.href = '/character-studio'; return; } setDestination(next); setSelectedApp(null); };
  const runAction = (action: HubAction) => setAudit(prev => [{ id: crypto.randomUUID(), time: new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'}), message: `${action} action requested from Hub`, state: 'OBSERVED' }, ...prev].slice(0, 10));
  const sendCommand = () => { const value = command.trim(); if (!value) return; setAudit(prev => [{ id: crypto.randomUUID(), time: 'NOW', message: `Local command requested: ${value}`, state: 'OBSERVED' }, ...prev].slice(0, 10)); setCommand(''); setDestination('TERMINAL'); };

  return <main className="apexHub">
    <header className="hub-topbar"><div className="hub-brand"><span className="hub-mark">A</span><div><strong>APEX TERMINAL</strong><small>REAL-TIME ENGINE</small></div></div>
      ...
      <div className="hub-truth"><CircleDot size={11}/> TRUTH: OBSERVED</div><span className="hub-clock">LOCAL</span><div className="hub-owner"><strong>GODSPEED</strong><small>OWNER</small></div><ChevronDown size={16}/>
    </header>
    <div className="hub-body">
      <aside className={`hub-left ${leftOpen?'':'closed'}`}><button className="hub-collapse" onClick={()=>setLeftOpen(!leftOpen)}>{leftOpen?<ChevronLeft/>:<ChevronRight/>}</button>{leftOpen && <>
        <div className="hub-section-label">WORKSPACE</div>{hubModules.map(m=><button key={m.id} className={`hub-nav ${destination===m.destination?'active':''}`} onClick={()=>navigate(m.destination)}><Layers size={15}/>{m.label}</button>)}
        <div className="hub-section-label">SYSTEM</div><button className={`hub-nav ${destination==='TERMINAL'?'active':''}`} onClick={()=>navigate('TERMINAL')}><TerminalSquare size={15}/>TERMINAL</button><button className={`hub-nav ${destination==='VAULT'?'active':''}`} onClick={()=>navigate('VAULT')}><LockKeyhole size={15}/>OMNI VAULT</button><button className="hub-nav" onClick={()=>navigate('CHARACTER_STUDIO')}><Box size={15}/>CHARACTER STUDIO</button>
      </>}</aside>
      <section className="hub-center"><div className="hub-center-head"><div><strong>{destination === 'DASHBOARD' ? 'APEX HUB' : destination.replaceAll('_',' ')}</strong><small>CANONICAL WORKSPACE · LOCAL-FIRST</small></div><div className="hub-project">PROJECT: <b>GODSPEED</b><span className="hub-observed">OBSERVED</span></div></div>
        <div className="hub-center-grid"><div className="hub-main-column">{destination === 'DASHBOARD' ? <>
          <ConnectedAppsPanel onOpen={app=>setSelectedApp(app)}/><div className="hub-lower-grid">
            ...
            <section className="hub-panel engines"><div className="hub-panel-head"><div><span className="eyebrow">ENGINES</span><strong>APEX EXECUTION LAYER</strong></div><Cpu size={16}/></div><div className="engine-grid">{[['APEX ENGINE',Activity],['APEX RENDER',MonitorCog],['PHYSICS',Box],['AUDIO ENGINE',Activity],['AI GENERATION',Sparkles],['WORLD BUILDER',Grid3X3]].map(([name,I])=>{const Icon=I as ElementType;return <button key={name as string} onClick={()=>setDestination('ENGINES')}><Icon size={17}/><span>{name as string}<small>MODULE</small></span></button>})}</div></section>
          </div></> : destination === 'CONNECTIONS' ? <ConnectedAppsPanel onOpen={app=>setSelectedApp(app)}/> : <WorkspaceCard destination={destination} onNavigate={navigate}/>}</div>
          <aside className={`hub-right ${gabbyOpen?'':'closed'}`}><button className="hub-right-toggle" onClick={()=>setGabbyOpen(!gabbyOpen)}>{gabbyOpen?<X size={15}/>:<Sparkles size={15}/>}</button>{gabbyOpen && <>
            <div className="gabby-head"><span>CONCIERGE: GABBY</span><Brain size={28}/></div><div className="gabby-avatar"><Brain size={42}/></div><p>Workspace context is loaded.</p><p className="muted">What are we building?</p>
            <div className="gabby-buttons"><button onClick={()=>setDestination('PROJECTS')}><Plus size={13}/> NEW PROJECT</button><button onClick={()=>setDestination('PROJECTS')}><ExternalLink size={13}/> OPEN PROJECT</button></div>
            ...
            <div className="gabby-actions"><button onClick={()=>navigate('CHARACTER_STUDIO')}><Box size={14}/> CHARACTER STUDIO</button><button onClick={()=>navigate('TERMINAL')}><TerminalSquare size={14}/> LOCAL TERMINAL</button><button onClick={()=>navigate('VAULT')}><LockKeyhole size={14}/> OMNI VAULT</button></div>
